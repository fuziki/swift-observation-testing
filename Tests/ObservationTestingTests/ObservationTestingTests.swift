...
// MARK: - Sample Production Code (ObservableObject / legacy Combine-based)
@MainActor
final class SampleLegacyViewModel: ObservableObject {
...
struct ObservationTestingTests {
    ...
    @Test("Can accurately record time progression and state changes for ObservableObject")
    func testObservableObjectTimelineRecording() async {
        // 1. Setup Timeline & ViewModel
        let timeline = TestTimeline()
        let vm = SampleLegacyViewModel(clock: timeline.anyClock)
    ...
    func testObservableObjectExpressionObservation() async {
        let timeline = TestTimeline()
        let vm = SampleLegacyViewModel(clock: timeline.anyClock)
